...
LOG_PATH    = "489_EventLogReport.txt"  # 入力ログ
BIN_SECONDS = 3600
TOTAL_HOURS = 12
EXCLUDE_BOX = True
EXCLUDE_DO = True
OUT_DIR     = "489_result" # 出力フォルダ
...
"""
# M_Hp,M_Hq,M_Hrだけ評価（M_H(road1)などを含めない）
def is_high_id(mid):
    if mid is None:
        return False
    s = str(mid).lower()
    return s.startswith("m_hp") or s.startswith("m_hq") or s.startswith("m_hr")
...
high_csv_path = os.path.join(OUT_DIR, "MH_hourly_firsttime_489.csv")
...
# 全体サマリ
low_summary_path = os.path.join(OUT_DIR, "ML_arrival_rate_489.csv")
...
low_byid_path = os.path.join(OUT_DIR, "ML_arrival_rate_by_id_489.csv")
